...
data class FileItem(
    ...
    companion object {
        ...
        private val LINK_EX_NAME = arrayOf(".lnk", ".LNK")

        fun create(parentDir: DirReader, file: File): FileItem {

            if (file.isDirectory) {
                return FileItem(parentDir, FileType.DIR, file.path, file.name)
            }
            ...
            if (fileType == FileType.LINK) {
                return create(parentDir, File(ShellLink(file).resolveTarget()))
            }

            return FileItem(parentDir, fileType, file.path, file.name)
        }
    }
}
